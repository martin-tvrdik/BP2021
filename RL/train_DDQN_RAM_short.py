from RL.DQNNetwork_RAM_short import DQNNetworkRAM
...
STEPS_SAVE = 500000  # frequency of model dump
...
policy_net = DQNNetworkRAM(N_INPUTS, N_ACTIONS).to(device)
target_net = DQNNetworkRAM(N_INPUTS, N_ACTIONS).to(device)
...
for s in range(STEPS):
    ...
    actions = policy_net(state.unsqueeze(0).to(device).float() / 255.)
    ...
    replay_memory.push(state, action, next_state, reward, int(not done))
    state = next_state
    ...
    if s > MIN_STEPS_UPDATE:
        ...
        if s % UPDATE_EVERY_N_STEPS == 0:
            ...
            if s % STEPS_SAVE == 0:
                ...
                torch.save(target_net, ENV_NAME + str(s) + "_short" + ".pt")
...
fig.savefig(ENV_NAME + "_rewards_short")
...
fig2.set_dpi(120)
fig2.savefig(ENV_NAME + "_scores_short")
